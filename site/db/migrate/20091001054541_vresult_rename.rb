class VresultRename < ActiveRecord::Migration
  def self.up
    ...
    create_view :pre_dataset_vresults, "select datasets.id dataset_id, count(runs.id) num_runs, runs.id last_run_id, min(runs.error) min_error from datasets inner join runs on datasets.id = runs.core_dataset_id group by datasets.id order by runs.updated_at desc" do |t|
      t.column :dataset_id
      t.column :num_runs
      t.column :last_run_id
      t.column :min_error
    end
    create_view :dataset_vresults, "select pre_dataset_vresults.*, runs.id best_run_id, runs.core_program_id best_core_program_id from pre_dataset_vresults inner join runs on runs.core_dataset_id = dataset_id where runs.error = min_error group by dataset_id" do |t|
      t.column :dataset_id
      t.column :num_runs
      t.column :last_run_id
      t.column :min_error
