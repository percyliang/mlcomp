...
class InteractiveLearningRunInfo < RunInfo
  attr_reader :interactor, :dataset, :stripper, :evaluator

  def initialize(interactor, dataset, stripper, evaluator)
  ...
  def self.defaultRunInfoSpecObj(domain, learner, dataset, tuneHyperparameters)
    ...
    Specification.new([self, learner, dataset, domain.utilsProgram, domain.evaluatorProgram])
  end

  def getRunSpecObj
    main = Program.findByName('interactive-learning', RunException)
    effectivePerformer = @domain.applyReduction(interactor, interactor.taskTypes[0])
    spec = RunSpecification.new([main, effectivePerformer, dataset, stripper, evaluator])
    spec.verifyTypes(RunException)
    spec
  end

  def coreProgram; interactor end
