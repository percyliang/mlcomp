...
# Addition of part-of-speech (POS) tagging accuracy:
#   Slav Petrov
# Acknowledgements: 
...
my ($word, $pos_g, $pos_s, $pos, $wp, $head_g, $dep_g, $head_s, $dep_s) ;
my (%counts, $err_pos, $err_head, $err_dep, $con, $con1, $con_pos, $con_pos1, $thresh) ;
...
{ # main reading loop
  ...
  { # loop on words
    ...
    ($word, $pos_g, $head_g, $dep_g)
      = @{$sent_gold[$i_w]}{'word', 'pos', 'head', 'dep'} ;
    $wp = $word.' / '.$pos_g ;

    # printf "%d: %s %s %s %s\n", $i_w,  $word, $pos_g, $head_g, $dep_g ;
    ...
    if (length($pos_g) > $max_pos_len)
    {
      $max_pos_len = length($pos_g) ;
    }

    ($pos_s, $head_s, $dep_s) = @{$sent_sys[$i_w]}{'pos', 'head', 'dep'} ;

    $counts{tot}++ ;
    $counts{word}{$wp}{tot}++ ;
    $counts{pos}{$pos_g}{tot}++ ;
    ...
    $err_pos = ($pos_g ne $pos_s) ;    # error in pos
	...
	$head_err = $head_s-$head_g ;
      ...
      $counts{pos}{$pos_g}{err_head}{tot}++ ;
      $counts{pos}{$pos_g}{err_head}{$head_err}++ ;
    }
    ...
    if ($err_dep)
    {
      ...
      $counts{pos}{$pos_g}{err_dep}{tot}++ ;
      $counts{pos}{$pos_g}{err_dep}{$dep_err}++ ;

      if ($err_head)
      {
	$counts{err_both}++ ;
	$counts{pos}{$pos_g}{err_both}++ ;
      }
    }
...
#printf OUT "  Part-of-speech tag score:   %d / %d * 100 = %.2f %%\n", 
#    $counts{tot}-$counts{err_pos}, $counts{tot}, 100-$counts{err_pos}*100.0/$counts{tot} ;

open (STATUS, '>status') || die "Could not open status file!";
printf STATUS "---\n";
